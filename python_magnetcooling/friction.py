...
class FrictionModel(ABC):
    ...
    @abstractmethod
    def compute(
        self, reynolds: float, hydraulic_diameter: float, friction_guess: float = 0.055
    ) -> float:
        """
...
class ColebrookFriction(FrictionModel):
    ...
    def compute(
        ...
        for i in range(max_iter):
            f_new = 1.0 / (-2.0 * log10(rel_roughness / 3.7 + 2.51 / (reynolds * sqrt(f)))) ** 2
        ...
        raise FrictionError(f"Colebrook iteration did not converge after {max_iter} iterations")
...
class SwameeFriction(FrictionModel):
    ...
    def compute(
        ...
        f = 1.325 / (ln(rel_roughness / 3.7 + 5.74 / exp(log(reynolds) * 0.9))) ** 2
...
def get_friction_model(
    name: str, roughness: float = 0.012e-3, constant_value: float = 0.055
) -> FrictionModel:
    """
