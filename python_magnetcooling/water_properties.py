...
class WaterProperties:
    ...
    def get_state(temperature: float, pressure: float) -> WaterState:
        ...
        try:
            ...
            raise WaterPropertiesError(f"Error calculating water properties: {e}") from e

    @staticmethod
    def compute_temperature_rise(
        flow_rate: float, power: float, temperature: float, pressure: float
    ) -> float:
        """
    ...
    def compute_reynolds(
        velocity: float, hydraulic_diameter: float, temperature: float, pressure: float
    ) -> float:
        """
